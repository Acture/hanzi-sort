...
fn main() {
    ...
    let jyutping_csv = data_dir.join("jyutping.csv");
    ...
    println!("cargo:rerun-if-env-changed=CARGO_FEATURE_COLLATOR_JYUTPING");
    if std::env::var_os("CARGO_FEATURE_COLLATOR_JYUTPING").is_some() {
        println!("cargo:rerun-if-changed={}", jyutping_csv.display());
    }
    ...
    if std::env::var_os("CARGO_FEATURE_COLLATOR_JYUTPING").is_some() {
        generate_jyutping_map(&jyutping_csv, &generated_dir.join("jyutping_map.rs"));
    }
}
...
fn generate_jyutping_map(data_csv: &Path, out_path: &Path) {
    ...
    for (index, result) in rdr.records().enumerate() {
        ...
        if record.len() != 3 {
            fail(&format!(
                "{}:{line}: expected exactly 3 columns (codepoint, jyutping, char), got {}",
                source.display(),
                record.len()
            ));
        }
        ...
        let jyutping_vec: Vec<&str> = jyutpings_field.split('|').collect();
        if jyutping_vec.iter().any(|s| s.is_empty()) {
            fail(&format!(
                "{}:{line}: empty jyutping entry for {code_str} ({jyutpings_field:?})",
                source.display()
            ));
        }

        let primary = jyutping_vec[0];
        if !primary.is_ascii() {
            fail(&format!(
                "{}:{line}: primary jyutping {primary:?} for {code_str} must be ASCII",
                source.display()
            ));
        }
        if primary.len() > 16 {
            fail(&format!(
                "{}:{line}: primary jyutping {primary:?} for {code_str} exceeds 16 bytes",
                source.display()
            ));
        }
        match primary.as_bytes().last() {
            Some(b'1'..=b'6') => {}
            _ => fail(&format!(
                "{}:{line}: primary jyutping {primary:?} for {code_str} must end with a tone \
                 digit 1-6; the convert_jyutping_to_csv.py pipeline appends 6 to rare toneless \
                 syllables — regenerate data/jyutping.csv",
                source.display()
            )),
        }
    ...
    writeln!(
        &mut writer,
        "pub static JYUTPING_MAP: ::phf::Map<u32, (char, &'static [&'static str])> = {};\n",
        map.build()
    )
    .unwrap_or_else(|error| fail(&format!("failed to write {}: {error}", out_path.display())));

    for required in [0x4E2D_u32, 0x6C49, 0x9999] {
        if !seen_codepoints.contains(&required) {
            fail(&format!(
                "required codepoint U+{required:04X} is missing from generated jyutping data; \
                 regenerate {} from the upstream Unihan kCantonese dataset",
                source.display()
            ));
        }
    }
