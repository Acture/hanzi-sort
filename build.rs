...
fn main() {
    ...
    {
        println!("cargo:rerun-if-env-changed=CARGO_FEATURE_COLLATOR_NAMES");
        if std::env::var("CARGO_FEATURE_COLLATOR_NAMES").is_ok() {
            let surname_csv = data_dir.join("surnames.csv");
            println!("cargo:rerun-if-changed={}", surname_csv.display());
            generate_surname_map(&surname_csv, &generated_dir.join("surname_map.rs"));
        }
    }
}

#[cfg(feature = "collator-names")]
fn generate_surname_map(data_csv: &Path, out_path: &Path) {
    ...
    // Hold the owned key/value strings alive until `map.build()` borrows them.
    ...
    for (index, result) in rdr.records().enumerate() {
        ...
        if record.len() != 2 {
            fail(&format!(
                "{}:{line}: expected exactly 2 columns (surname, reading), got {}",
                source.display(),
                record.len()
            ));
        }

        let surname = record[0].trim();
        let readings_field = record[1].trim();
        if surname.is_empty() {
            fail(&format!("{}:{line}: empty surname column", source.display()));
        }

        let readings: Vec<&str> = readings_field.split('|').map(str::trim).collect();
        let char_count = surname.chars().count();
        if readings.len() != char_count {
            fail(&format!(
                "{}:{line}: surname {surname:?} has {char_count} characters but {} readings",
                source.display(),
                readings.len()
            ));
        }
        for reading in &readings {
            if reading.is_empty() {
                fail(&format!(
                    "{}:{line}: empty reading for surname {surname:?}",
                    source.display()
                ));
            }
            if !reading.is_ascii() {
                fail(&format!(
                    "{}:{line}: reading {reading:?} for surname {surname:?} must be ASCII",
                    source.display()
                ));
            }
            if reading.len() > 16 {
                fail(&format!(
                    "{}:{line}: reading {reading:?} for surname {surname:?} exceeds 16 bytes",
                    source.display()
                ));
            }
            match reading.as_bytes().last() {
                Some(b'1'..=b'5') => {}
                _ => fail(&format!(
                    "{}:{line}: reading {reading:?} for surname {surname:?} must end with a tone \
                     digit 1-5 (use 5 for neutral tone)",
                    source.display()
                )),
            }
        }
        ...
        rows.push((surname.to_string(), value_str));
    }

    let mut map = Map::new();
    for (surname, value) in &rows {
        map.entry(surname.as_str(), value.as_str());
    }

    writeln!(
        &mut writer,
        "pub static SURNAME_MAP: ::phf::Map<&'static str, &'static [&'static str]> = {};\n",
        map.build()
    )
    .unwrap_or_else(|error| fail(&format!("failed to write {}: {error}", out_path.display())));
}
